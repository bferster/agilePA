...
require_once('config7.php');
	...
	$id=addEscapes($id,$link);									// Escape id
	...
	if (!mysqli_num_rows($result)) {							// If not found, add it
		...
		$query.=addEscapes($title,$link)."','";
		$query.=addEscapes($script,$link)."','";
		$query.=addEscapes($email,$link)."','";
		$query.=addEscapes($password,$link)."','";
		$query.=addEscapes($ver,$link)."','";
		$query.=addEscapes($private,$link)."')";
		$result=mysqli_query($link, $query);					// Run query
		...
		$id=addEscapes($id,$link);								// Escape id
		if ($id != "") {										// If valid
			$query="UPDATE qshow SET title='".addEscapes($title,$link)."' WHERE id = '".$id."'";
			$result=mysqli_query($link, $query);				// Run query
			$query="UPDATE qshow SET script='".addEscapes($script,$link)."' WHERE id = '".$id."'";
			$result=mysqli_query($link, $query);				// Run query
			$query="UPDATE qshow SET private='".addEscapes($private,$link)."' WHERE id = '".$id."'";
			$result=mysqli_query($link, $query);				// Run query
			$query="UPDATE qshow SET deleted='".addEscapes($deleted,$link)."' WHERE id = '".$id."'";
			$result=mysqli_query($link, $query);				// Run query
	...
	function addEscapes($str, $lnk)							// ESCAPE ENTRIES
	{
		if (!$str)												// If nothing
			return $str;										// Quit
